...
版本: v1.6.0.1 | 许可证: CC BY-NC-SA 4.0 | 语言: 🇨🇳 中文 / 🇺🇸 English
...
- **功能**：使用深度学习自动去除图像背景
- **参数调节**：
  - **阈值设置**：自动/手动模式，精确控制前景/背景分离
  - **边缘羽化**：调节边缘柔和程度（0-100），让抠图更自然
  - **背景颜色**：选择白色/黑色/透明背景
- **实时预览**：调整参数后自动更新预览，所见即所得
- **使用方法**：
...
# 模型会自动下载到 data/model/ 目录
```
如果是手动下载，需要将模型放在程序目录下的"./data/model/" 并改名为"RMBG-\*.onnx" ('\*' 可替换)

**注意**：
- AI 功能是可选的，不安装也不影响其他功能
- 首次使用会提示用户下载模型，点击下载即可
