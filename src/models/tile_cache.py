...
class TileCache:
    ...
    def set_image(self, pixels: np.ndarray, selection_mask: np.ndarray | None = None):
        """
        设置图片数据
    ...
    def update_image(self, pixels: np.ndarray, selection_mask: np.ndarray | None = None):
        """
        更新图片数据但不清空缓存
    ...
    def get_tile(self, tile_x: int, tile_y: int) -> QPixmap | None:
        """
        获取指定块的 QPixmap
    ...
    def _generate_tile(self, tile_x: int, tile_y: int) -> QPixmap | None:
        """
        生成指定块的 QPixmap
