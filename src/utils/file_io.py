...
def load_image(file_path: str, binarize: bool = True, 
    ...
    try:
        ...
        if isinstance(e, (FileNotFoundError, ValueError)):
            raise
        raise IOError(f"无法读取图片文件: {file_path}。错误: {str(e)}")


def save_image(image_data: ImageData, file_path: str, format: Optional[str] = None):
    """
    保存图片文件
    ...
    try:
        ...
        raise IOError(f"无法保存图片文件: {file_path}。错误: {str(e)}")
